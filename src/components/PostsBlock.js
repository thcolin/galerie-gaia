...
export default class PostsBlock extends React.Component {
  render () {
    const displayed = _.orderBy(getPages(this.props.pageContext.pages, '/posts'), 'frontmatter.date', 'desc')
    const recents = displayed.slice(0, _.get(this.props, 'section.num_posts_displayed'))
    return (
            ...
            <div className='block-content'>
              ...
              {_.get(this.props, 'section.actions') &&
                <p className='block-cta'>
                  {_.map(_.get(this.props, 'section.actions'), (action, index) => (
                    <Link key={index} to={safePrefix(_.get(action, 'url'))} className='button'>{_.get(action, 'label')}</Link>
                  ))}
                </p>}
            </div>
          </div>
