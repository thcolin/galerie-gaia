...
export default class PortfolioBlock extends React.Component {
  render () {
    const displayed = _.orderBy(getPages(this.props.pageContext.pages, '/projects'), 'frontmatter.date', 'desc')
    const recents = displayed.slice(0, _.get(this.props, 'section.num_projects_displayed'))
    const length = _.size(recents)
    return (
          ...
          <div className='block-content'>
            <div className={'portfolio-feed layout-' + _.get(this.props, 'section.layout_style')}>
              {_.map(recents, (post, index) => (
                <article key={index} className='post project'>
                  {(index === length - 1) ? (
